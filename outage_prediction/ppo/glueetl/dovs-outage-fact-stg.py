##/**------------------------------------------------------------------------------------------**/
##/**          AMERICAN ELECTRIC POWER - Dovs Fact Data                              **/
##/**------------------------------------------------------------------------------------------**/
...
##/**   V0.1      Girish        05/11/2023  script for dovs-outage-fact-stg            **/
##/**                                                                                          **/
##/**------------------------------------------------------------------------------------------**/
## s3://aep-datalake-apps-dev/hdpapp/cties/glueetl/dovs-outage-fact-stg.py              **/
##==============================================================================================##
## Import Libraries 
...
final_path = s3_data_work+"/raw/dovs_eems/dovs_outage_fact_stg/"
##==============================================================================================##
...
try:
    ...
    ##==============================================================================================##
    ## Get meter fact data from ansersds through jdbc

    df_dovs_fact=sqlContext.read.format("jdbc") \
    .option("url",ConnectString).option("driver",DBDriver) \
    .option("fetchsize",100000) \
    .option("dbtable","(SELECT * FROM ANSERSDS.aep_dl_outage_fact_dovs_vw)" ) \
    ...
    df_dovs_fact=df_dovs_fact.withColumn("SURR_KEY_NB",col("SURR_KEY_NB").cast("double"))\
    .withColumn("DT_ON_TS",col("DT_ON_TS").cast("string"))\
    .withColumn("DT_OFF_TS",to_date(col("DT_OFF_TS"),'yyyy-MM-dd').cast("string"))\
    .withColumn("APRVD_DT_TS",col("APRVD_DT_TS").cast("string"))\
    .withColumn("ATMPT_TO_DSPTCH_DT",col("ATMPT_TO_DSPTCH_DT").cast("string"))\
    .withColumn("OMS_CLOSE_TIME",col("OMS_CLOSE_TIME").cast("string"))\
    .withColumn("LAST_EDIT_DT_TS",col("LAST_EDIT_DT_TS").cast("string"))
    
    df_dovs_fact.printSchema()    

    ##==============================================================================================##
    ##Load the data in staging table

    df_dovs_fact.write.mode("overwrite") \
    ...
    print(e)
    msg_title=f"GlueJob: dovs-outage-fact-stg failed"
    print(msg_title)
    msg_body=f"## GlueJob: dovs-outage-fact-stg.py failed while processing"
    print(msg_body)
