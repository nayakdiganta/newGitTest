...
function drop_create_tables()
{
	...
	## 1. stg_meterevents.end_device_event_errors tbl drop
	...
	#1. create stg_meterevents.end_device_event_errors table
	...
	TBLPROPERTIES (
	'bucketing_version'='2', 
	'orc.bloom.filter.columns'='issuertracking_id,serialnumber', 
	...
	##===========================================
	## Refresh error table with new schema

	## 1. stg_meterevents.end_device_event_errors tbl msck
	end_device_event_errors_tbl_msck_qry='MSCK REPAIR TABLE stg_meterevents.end_device_event_errors'
	echo "end_device_event_errors_tbl_msck_qry: $end_device_event_errors_tbl_msck_qry"
	end_device_event_errors_tbl_msck_qry_eid=$(aws athena start-query-execution --region $aws_region --query-string "$end_device_event_errors_tbl_msck_qry" --result-configuration "OutputLocation=$s3_temp_loc" --work-group ${ATHENA_WORK_GROUP} --query "QueryExecutionId" |sed 's/"//g')
	echo "end_device_event_errors_tbl_msck_qry_eid: $end_device_event_errors_tbl_msck_qry_eid"
	sleep 5
	end_device_event_errors_tbl_msck_qry_status=$(aws athena get-query-execution --query-execution-id $end_device_event_errors_tbl_msck_qry_eid --query "QueryExecution.Status.State" |sed 's/"//g')
	
	if [ "${end_device_event_errors_tbl_msck_qry_status}" == "SUCCEEDED" ]; then
		fnLog "stg_meterevents.end_device_event_errors table msck repair succeeded."
	else
		fnLog "stg_meterevents.end_device_event_errors table msck repair failed."
		log_error_exit
	fi
