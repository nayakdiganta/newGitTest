##/**------------------------------------------------------------------------------------------**/
##/**          AMERICAN ELECTRIC POWER - Dovs Premise Data                              **/
##/**------------------------------------------------------------------------------------------**/
...
##/**   V0.1      Girish        05/11/2023  script for dovs-outage-premise-stg            **/
##/**                                                                                          **/
##/**------------------------------------------------------------------------------------------**/
## s3://aep-datalake-apps-dev/hdpapp/cties/glueetl/dovs-outage-premise-stg.py              **/
##==============================================================================================##
## Import Libraries 
...
final_path = s3_data_work+"/raw/dovs_eems/dovs_outage_premise_stg/"
##==============================================================================================##
...
try:
    ...
    df_dovs_premise=sqlContext.read.format("jdbc") \
    .option("url",ConnectString).option("driver",DBDriver) \
    .option("fetchsize",100000) \
    .option("dbtable","(SELECT * FROM ANSERSDS.aep_dl_outage_premise_dovs_vw)" ) \
    ...
    df_dovs_premise=df_dovs_premise.withColumn("OFF_TM",col("OFF_TM").cast("string"))\
    .withColumn("REST_TM",col("REST_TM").cast("string"))\
    .withColumn("LATITUDE_NB",col("LATITUDE_NB").cast("double"))\
    .withColumn("LONGITUDE_NB",col("LONGITUDE_NB").cast("double"))
    
    df_dovs_premise.printSchema()    

    ##==============================================================================================##
    ##Load the data in staging table

    df_dovs_premise.write.mode("overwrite") \
    ...
    print(e)
    msg_title=f"GlueJob: dovs-outage-premise-stg failed"
    print(msg_title)
    msg_body=f"## GlueJob: dovs-outage-premise-stg.py failed while processing"
    print(msg_body)
